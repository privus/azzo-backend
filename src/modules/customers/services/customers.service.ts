...
export class CustomersService implements ICustomersRepository{
  ...
  async syncCustomersPriceTables(): Promise<void> {
    ...
    for (const cliente of clientes) {
      try {
        const url = `${this.apiUrlSellentt}${this.storeTag}/${cliente.codigo}`;
        console.log(`📡 Buscando tabela de preço para cliente código ${cliente.codigo} → ${url}`);
        ...
        if (!clientData) {
          console.warn(`⚠️ Nenhum dado retornado para cliente ${cliente.codigo}.`);
          continue;
        }

        cliente.cod_tabela_preco = Number(clientData.default_price_table.code);
        await this.clienteRepository.save(cliente);
  
        console.log(`✅ Cliente ${cliente.codigo} atualizado com tabela ${cliente.cod_tabela_preco}`);
  
        await delay(3000);
  
      } catch (error: any) {
        console.error(`❌ Erro ao atualizar cliente ${cliente.codigo}:`, error.message);
